...
def build_and_evaluate_model(train_with, num_train_epochs=6, seed=42, per_device_train_batch_size=10, per_device_eval_batch_size=128,
    ...
    file_name = f'./results/{train_with}_resInfo_{str(current_time)}_epochs{num_train_epochs}_lr{learning_rate}.txt'
    f = open(file_name, "a")
...
if __name__ == '__main__':
    for lr in [1e-5, 3e-5, 6e-5]:
        for n_epochs in range(3, 9):
            for ds_name in tokenized_ds:
                if ('dev' not in ds_name) and ('test' not in ds_name):
                    build_and_evaluate_model(ds_name, num_train_epochs=n_epochs, learning_rate=lr)
